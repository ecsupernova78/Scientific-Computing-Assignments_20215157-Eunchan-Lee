...
BigNum::BigNum (char *c, char s)
{
  ...
  if (mod > 0)
  {
    ...
    for (i = temp; i >= 0; i -= 4)
  v[index] = v[index]
    ^ (static_cast<unsigned long int> (C2I (*c++))
...
BigNum operator + (const BigNum &x, const BigNum &y)
{
  ...
  for (i = 0; i < smallersize; i++)
  {
    ...
    *zptr ^= *xptr ^ *yptr;
	...
	{
	  andtemp = carry & *zptr;
	  if ((Test (carry, NUMBITS - 1) == 1)
	      && (Test (*zptr, NUMBITS - 1) == 1))
	    {
	      *zptr = Set (*(++zptr), 0);
	      *(zptr--);
	    }
...
BigNum operator / (const BigNum &x, const BigNum &y)
{
  ...
  while (true)
  {
   ...
   {
    /* difference is non-zero positive */
    z = z << 1;
    z = z ^ 1;
    tempbits = xbits - ybits;
    if (prevneg == false)
...
BigNum operator >> (BigNum &x, unsigned long int index)
{
  ...
  *(zptr--) = *(xptr--);
  ...
  {
   if (GetNumBits (*xptr) < static_cast<int> (zfirstbits % NUMBITS)
     || ((xbits - index) % NUMBITS == 0))
   {
...
int Test (const unsigned long int num, int index)
{
  return 0
	  ...
	  & (static_cast<unsigned long int> (1)
	      << static_cast<unsigned long int> (index)));
}
