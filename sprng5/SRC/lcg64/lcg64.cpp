...
unsigned int PARAMLIST[NPARAMS][2] =
  {
...
int LCG64::init_rng (int gn, int tg, int s, int p)
{
  ...
  if (tg <= 0) /* Is total_gen valid ? */
  {
    tg = 1;
    fprintf (stderr,
      "WARNING - lcg64 init_rng: Total_gen <= 0. Default value of 1 used for total_gen\n");
  }

  if (gn >= MAX_STREAMS) /* check if gen_num is valid    */
    fprintf (stderr,
      "WARNING - lcg64 init_rng: gennum: %d > maximum number of independent streams: %d\n\tIndependence of streams cannot be guranteed.\n",
      gn, MAX_STREAMS);

  if (gn < 0 || gn >= tg) /* check if gen_num is valid    */
  {
    fprintf (stderr,
       "ERROR - lcg64 init_rng: gennum %d out of range [%d,%d).\n", gn,
       0, tg);
    return 0;
  }
  ...
  {
    fprintf (stderr,
      "WARNING - lcg64 init_rng: parameter not valid. Using Default parameter.\n");
    p = 0;
  }
  ...
  multiplier[1] = (double) (PARAMLIST[p][0] >> 22
    | (PARAMLIST[p][1] & 0xfff) << 10);
  multiplier[2] = (double) (PARAMLIST[p][1] >> 12);
  state[0] = (double) ((INIT_SEED0 ^ gn) & 0x3fffff);
  state[1] = (double) ((INIT_SEED0 ^ gn) >> 22
    | ((INIT_SEED1 ^ (unsigned) s << 1) & 0xfff) << 10);
...
void LCG64::advance_state ()
{
  ...
  st1 = t1 - TWO_P22 * t2;
  assert ((int) st1 == st1);
  t2 += state[2] * multiplier[0] + state[1] * multiplier[1]
    + state[0] * multiplier[2];
