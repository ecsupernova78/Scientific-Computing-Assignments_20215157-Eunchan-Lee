...
static const uint64 MASK64 =
  { 0xffffffffU, 0xffffffffU };
static const uint64 MINUS1 =
  { 0xffffffffU, 0xffffffffU }; /* -1 (mod 2^(BITS-2)) */
static uint64 ONE =
  { 0x1U, 0x0U };
...
#else
static const uint64 INT_MOD_MASK =
  { 0xffffffffU, 0x3fffffffU };
static const uint64 INT_MASK =
  { 0xffffffffU, 0x1fffffffU };
static const uint64 INTX2_MASK =
  { 0xffffffffU, 0x0fffffffU };
static const uint64 SEED_MASK =
  { 0x5a38, 0x0U };
/*
...
static void si_double (uint64 *a, uint64 *b, int length)
{
  ...
  if (notzero(temp1))
    fprintf (stderr,
      "WARNING: si_double -- RNG has branched maximum number of times.\n\t Independence of generators no longer guaranteed\n");
...
int MLFG::init_rng (int gn, int tg, int s, int pa)
{
  ...
  if (tg <= 0) /* Is total_gen valid ? */
  {
    tg = 1;
    fprintf (stderr,
      "WARNING - init_rng: Total_gen <= 0. Default value of 1 used for total_gen\n");
  }

  if (gn >= MAX_STREAMS) /* check if gen_num is valid    */
    fprintf (stderr,
      "WARNING - init_rng: gennum: %d > maximum number of independent streams: %d\n\tIndependence of streams cannot be guranteed.\n",
      gn, MAX_STREAMS);
  ...
  {
    fprintf (stderr,
      "WARNING - init_rng: parameter not valid. Using Default parameter.\n");
    pa = 0;
  }
...
int MLFG::spawn_rng (int nspawned, Sprng ***newgens)
{
  ...
  if (nspawned <= 0) /* is nspawned valid ? */
    {
      nspawned = 1;
      fprintf (stderr,
        "WARNING - spawn_rng: nspawned <= 0. Default value of 1 used for nspawned\n");
    }
  ...
  for (i = 0; i < nspawned; i++)
  {
    ...
    genptr[i]->array_sizes[0] = genptr[i]->lval * sizeof(uint64)
      / sizeof(int);
    genptr[i]->array_sizes[1] = (genptr[i]->lval - 1) * sizeof(uint64)
      / sizeof(int);
  }
