...
/* Compare s1 and s2 for n characters */
...
int LCG::init_rng (int gn, int tg, int s, int m)
{
  ...
  {
    tg = 1;
    errprint ("WARNING", "init_rng",
      "Total_gen <= 0. Default value of 1 used for total_gen");
  }
  ...
  {
    errprint ("WARNING", "init_rng",
      "multiplier not valid. Using Default param");
    m = 0;
  }
...
int LCG::spawn_rng (int nspawned, Sprng ***newgens)
{
  ...
  {
    nspawned = 1;
    errprint ("WARNING", "spawn_rng",
      "nspawned <= 0. Default value of 1 used for nspawned");
  }
  ...
  for (i = 0; i < nspawned; i++)
  {
    ...
    {
      fprintf (stderr,
        "WARNING - spawn_rng: gennum: %d > maximum number of independent streams: %d\n\tIndependence of streams cannot be guranteed.\n",
...
void mult_48_64 (unsigned long int *a, unsigned long int *b,
		 unsigned long int *c)
{
#ifdef LONG64
  *c = (*a * *b);
...
void LCG::multiply (int *m, int *s, int *res)
{
  ...
  seed[0] = ((unsigned int) seed[1] >> 24) + res[2]
      + ((unsigned int) res[1] >> 12) + (res[3] << 12);
  seed[1] &= 16777215;
  seed[0] &= 16777215;
}
#endif
...
void LCG::advance_seed ()
{
  ...
  switch (i)
    ...
    default:
      fprintf (stderr,
        "WARNING: advance_seed parameters for multiplier %d not available\n",
        i);
      return;
    }
  ...
  temp2[0] = prime % (1 << 12);
  temp2[1] = (prime >> 12) % (1 << 12);
  temp2[2] = (prime >> 24) % (1 << 12);
  temp2[3] = 0; /* prime */
...
int LCG::unpack_rng (char *packed)
{
  ...
  seed[1] = m2[1] & 0xffffff;
  seed[0] = m2[1] >> 24 | m2[0] << 8;
  p += load_int (p, 4, (unsigned int*) &init_seed);
  p += load_int (p, 4, (unsigned int*) &prime);
  p += load_int (p, 4, (unsigned int*) &prime_position);
  p += load_int (p, 4, (unsigned int*) &prime_next);
