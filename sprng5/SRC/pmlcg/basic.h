...
extern struct BIGNUM_ARRAY_TYPE join (struct BIGNUM_ARRAY_TYPE first,
				      struct BIGNUM_ARRAY_TYPE second)
{
...
extern struct BIGNUM_ARRAY_TYPE mult_reduce (BigNum *number,
					     struct BIGNUM_ARRAY_TYPE set,
					     BigNum *limit)
{
  struct BIGNUM_ARRAY_TYPE result;
  long count, count2;
...
extern struct BIGNUM_ARRAY_TYPE find_denom (BigNum *the_primes, long maxsize,
					    BigNum *limit)
{
...
extern void mu_eval (BigNum *result, BigNum *x,
		     struct BIGNUM_ARRAY_TYPE denominators)
{
  BigNum val;
  long count = 0;
  ...
  *result = Set_ui (0ul);
  ...
  while ((count < denominators.size)
      && (b_cmp (val, static_cast<unsigned long int> (0)) != 0))
    {
...
extern BigNum linear_find (BigNum *y, BigNum *current_mu, BigNum *guess,
			   struct BIGNUM_ARRAY_TYPE the_primes)
{
  while (b_cmp (*current_mu, *y) < 0)
  {
    ...
    //redid this to eliminate an empty if branch
...
extern void Mu (BigNum *result, BigNum *x,
		struct BIGNUM_ARRAY_TYPE left_to_kill,
		struct BIGNUM_ARRAY_TYPE the_final_denom)
{
...
extern BigNum find_M (BigNum *guess, BigNum *y,
		      struct BIGNUM_ARRAY_TYPE left_to_kill,
		      struct BIGNUM_ARRAY_TYPE denom,
		      struct BIGNUM_ARRAY_TYPE initial_primes, BigRat *magic)
{
...
extern int rel_prime (BigNum *result, BigNum *number, REL_PRIME_TABLE data)
{
  ...
  *result = Set_ui (0ul);
  ...
  if (b_cmp (*(data.valid), *number) >= 0)
  {
    ...
    *result = find_M (&guess, number, data.kill_us, data.denom,
