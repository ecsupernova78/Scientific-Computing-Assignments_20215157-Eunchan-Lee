...
long init_tests (int argc, char *argv[])
{
  ...
  if (argc < 7 + 1)
    {
      fprintf (
	  stderr,
	  "Usage: %s n_sets ncombine seed param nsubsequences skip test_arguments\n",
	  argv[0]);
      exit (-1);
    }
  ...
  if (atoi (argv[1]) > 5 || atoi (argv[1]) < 0)
    {
      fprintf (
	  stderr,
	  "Error: First command line argument(random number type) should be between 0 to 5\n");
      exit (-1);
    }
  ...
  if (atoi (argv[3]) <= 0)
    {
      fprintf (stderr,
	       "Error: Third command line argument should be greater than 0\n");
      exit (-1);
    }
  ...
  if (atoi (argv[6]) <= 0)
    {
      fprintf (stderr,
	       "Error: Sixth command line argument should be greater than 0\n");
      exit (-1);
    }
  /*-------------- End by changing ------------------------------*/
  ...
  for (i = 0; i < n_combine; i++)
    {
      ...
      gens[i]->init_sprng (n_combine * current_group + i + OFFSET,
			   n_combine * n + OFFSET, seed, init_param);
    }
...
void next_stream (void)
{
  ...
  if (current_subsequence > 0)
	...
	gens[i]->free_sprng ();
	  ...
	  && current_group < first_group + init_ngroups)
	for (i = 0; i < n_combine; i++)
	  {
	    gens[i] = SelectType (rng_type);
	    gens[i]->init_sprng (n_combine * current_group + i + OFFSET,
				 n_combine * init_total + OFFSET, init_seed,
				 init_param);
	  }
      else if (current_group > first_group
	  && current_group > first_group + init_ngroups)
