...
void compute (int i, int N) /* print results */
{
  ...
  printf ("%5d.   %.6f   %.6f   %.6f   %.6f   %.6f   %.6f   %.6f\n", N,
	  Energy[i + 1][0], fabs (Energy[i + 1][0] - exact_energy),
...
void wolff (int block_size, int use_blocks, int multi, int modul)
{
  ...
  printf (
      "N     Energy  Energy_err  Sigma_Enrgy    Cv   Cv_error  Sigma_Cv   sqrtNInv\n");
...
int main (int argc, char **argv)
{
  ...
  if (argc == 7) /*--- increase argc by 1 ---*/
    ...
    {
      printf (
	  "USAGE: %s seed multi modul block_size discard_blocks use_blocks\n",
	  argv[0]);
      exit (-1);
    }
