  ...
  for (i = 1; i < 150; i++)
    {
      ...
      percent += term;
      if (fabs (term) < 0.005 * fabs (previous_term)
	  || fabs (term) < 0.0000001 * percent)
	return 1.0 - percent;
...
double chisquare (long *actual, double *probability, long n, long k, int *nb)
{
  ...
  for (i = n_actual = nbins = 0, prob = 0.0; i < k; i++)
    {
      ...
      if (n * prob >= 5 || i == k - 1)
	{
	  V += (n_actual - n * prob) * (n_actual - n * prob) / (n * prob);
	  nbins++;
	  n_actual = 0;
	  prob = 0.0;
	}
  ...
  *nb = nbins;
  ...
  if (nbins == 1)
    ...
    fprintf (
	stderr,
	"WARNING: The effective number of bins in Chisquare: %ld, was less than the given number of bins: %ld, due to the effect of combining bins in order to make the expected number of hits per bin sufficiently large.\n",
	nbins, k);
...
double cum_normal (double sample, double mean, double stddev)
{
  ...
  g = 1.0 / (1.0 + f * temp);
  h = 1.0
      - (0.39894228040143 * exp (-0.5 * temp * temp))
	  * (a * g + b * g * g + c * g * g * g + d * g * g * g * g
	      + e * g * g * g * g * g);
