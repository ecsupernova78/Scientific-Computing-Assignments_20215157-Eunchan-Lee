...
void initTest (int argc, char *argv[])
{
  ...
  if ((maxGapLen < 0) || (lowerBound < 0.0) || (upperBound <= lowerBound)
      || (1.0 < upperBound) || (numTotGap <= 0))
    {
...
int main (int argc, char *argv[])
{
  ...
  for (curRound = 0; curRound < numRepeat; curRound++)
	...
	bins[index] = 0;
      for (index = 0; index < numTotGap; index++)
	PROC_ONE_GAP
      ;

      chiSqrs[curRound] = chisquare (bins, probs, numTotGap, numBin,
				     &Bins_used);
