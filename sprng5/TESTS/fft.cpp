...
void Analyze (int nstreams, int nruns, int n)
{
  ...
  if (fabs (means[0] - mean1) > mult * StdDev1) /* Check constant term            */
    {
      count++;
      printf (
	  "%d. (0,0) th coefficient exceeds ~ %g th percentile: Expected = %g, observed = %g, StdDev = %g\n\n",
  ...
  for (j = 0; j < nstreams; j++) /*  Check each coefficient                    */
	...
	if (i == 0 && j == 0) /* The constant term has already been checked */
	  ...
	  {
	    if (nstreams % 2 == 1 || j % (nstreams / 2) != 0
		|| i % (n / 2) != 0)
	      StdDev = StdDev2;
	    else
	      StdDev = StdDev1;
	...
	if (fabs (means[2 * i + j * lda] - mean2) > mult * StdDev) /* Real part of coeff. */
	  {
	    count++;
	    printf (
		"%d. Real part of (%d,%d) th coefficient exceeds ~ %g th percentile: Expected = %g, observed = %g, StdDev = %g\n\n",
	...
	if (fabs (means[2 * i + j * lda + 1] - mean2) > mult * StdDev) /* Imaginary part    */
	  {
	    count++;
	    printf (
		"%d. Complex part of (%d,%d) th coefficient exceeds ~ %g th percentile: Expected = %g, observed = %g, StdDev = %g\n\n",
		count, i, j, percentile, mean2, means[2 * i + j * lda + 1],
		StdDev);
	  }
      }
